...
- 支払い情報（当社の決済サービスプロバイダーであるStripeが処理）
...
### 1.4 アフィリエイトの帰属
アフィリエイトリンクからアクセスした場合や関連する紹介ページを利用した場合、TheAffs Affiliate SDKはアフィリエイト、クリック、訪問者、参照元、サブパートナーの識別子を`cleanclip.cc`ドメインのCookieに保存することがあります。ブラウザは`cleanclip.cc`および該当する決済用サブドメインへのリクエスト時にこれらのCookieを送信し、注文の紹介元を判定できるようにします。これらの識別子にクリップボードの内容は含まれません。
...
## 3. お客様の個人情報の共有・転送・開示方法
当社は、サービスの提供に必要な範囲に限って以下のサービス事業者に情報を処理させます：

- Stripeは支払いおよび決済情報を処理します。
- TheAffsは紹介と注文を関連付けるためのアフィリエイト帰属識別子を処理します。

いずれのサービスにも、CleanClipからクリップボードの内容は提供されません。

当社は適用されるデータ保護法に基づいて個人情報を取り扱います。また、適用法で許可または要求され、関連する法的条件および手続きが満たされる場合、以下の状況で個人情報を共有・転送・開示することがあります：
